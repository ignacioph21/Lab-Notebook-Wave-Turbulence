...
\begin{figure}[th!]
	\centering
	\includegraphics[width=0.87\linewidth]{Figures/08_12_2025/joint_level_curves}
...
\begin{figure}[!ht]
	\centering
	\includegraphics[width=0.987\linewidth]{Figures/08_12_2025/Progresión}
	\caption{Curvas que encierran el 67 \% de los puntos para los distintos forzados y curva $S_{\text{lím}}$. }
...
\begin{equation}
	S_{\text{lím}} = \frac{H_{\text{máx}}}{L(T)} \approx \frac{1}{7} \tanh{k(T)d} 
\end{equation}
...
\begin{figure}[!ht]
	...
	\end{subfigure}
	\end{minipage}
	\caption{Algunas distribuciones de probabilidad $S/S_\text{rms}$ para las pdfs promediadas en ventanas temporales con overlap. La cantidad de bins es la raíz cuadrada de la cantidad de ondas individuales en cada histograma. Para el ajuste se tomaron los puntos centrales de los bins como coordenada x. Faltaría calcular el $\chi_\nu^2$ y $R^2$ para saber que Tan bien dio el Ajuste. } % t 
...
\begin{figure}[!ht]
	\begin{minipage}[c]{0.5\textwidth}
			\begin{subfigure}{\textwidth}
					...
					\captionsetup{width=0.8\textwidth}
					\subcaption{Relación de dispersión gravedad (aguas profundas).}
					% \label{fig:}
				\end{subfigure}
		\end{minipage}\begin{minipage}[c]{0.49\textwidth} 
			\begin{subfigure}{\textwidth}
					...
					\captionsetup{width=0.8\textwidth}
					\subcaption{Relación de dispersión completa (profundidad $d$).}
					% \label{fig:}
				\end{subfigure}
		\end{minipage}
	\caption{Mean Wave Steepness para los distintos forzados utilizando distintas relaciones de dispersión en los datos. }
	\label{fig:}
\end{figure}
...
Se puede notar cómo se va prendiendo la cascada de energía directa y se van poblando las distintas escalas del flujo a medida que pasa el tiempo. % a    
